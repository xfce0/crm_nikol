...
async def get_categories():
    ...
    try:
        with get_sync_db_context() as sync_db:
            categories = sync_db.query(Portfolio.category).filter(
            ...
            result = [
                {
                    ...
                    "count": sync_db.query(Portfolio).filter(
...
async def get_portfolio_item(project_id: int):
    """Получить конкретный проект портфолио"""
    try:
        with get_sync_db_context() as sync_db:
            project = sync_db.query(Portfolio).filter(Portfolio.id == project_id).first()
...
async def create_portfolio_item(request: Request):
    ...
    try:
        ...
        with get_sync_db_context() as sync_db:
            ...
            sync_db.add(new_project)
            sync_db.commit()
            sync_db.refresh(new_project)
...
async def delete_portfolio_item(project_id: int):
...
async def reorder_portfolio(order_data: dict):
...
async def get_portfolio_stats():
    ...
    try:
        with get_sync_db_context() as sync_db:
            total_projects = sync_db.query(Portfolio).count()
            visible_projects = sync_db.query(Portfolio).filter(Portfolio.is_visible == True).count()
            featured_projects = sync_db.query(Portfolio).filter(Portfolio.is_featured == True).count()
            total_views = sync_db.query(Portfolio).with_entities(Portfolio.views_count).all()
            total_views_sum = sum([v[0] for v in total_views if v[0]]) if total_views else 0

            # Статистика по категориям
            categories_stats = sync_db.query(
                Portfolio.category,
                Portfolio.id
...
async def get_public_portfolio_list(
    ...
    try:
        with get_sync_db_context() as sync_db:
            query = sync_db.query(Portfolio).filter(Portfolio.is_visible == True)
...
async def get_public_portfolio_item(project_id: int):
    ...
    try:
        with get_sync_db_context() as sync_db:
            project = sync_db.query(Portfolio).filter(
...
async def like_portfolio_item(project_id: int):
...
async def publish_to_telegram(
    ...
    try:
        ...
        with get_sync_db_context() as sync_db:
            ...
            portfolio_item = sync_db.query(Portfolio).filter(Portfolio.id == portfolio_id).first()
            ...
            result = await portfolio_telegram_service.publish_portfolio_item(portfolio_item, sync_db)
...
async def update_published_item(
    ...
    try:
        ...
        with get_sync_db_context() as sync_db:
            ...
            result = await portfolio_telegram_service.update_published_item(portfolio_item, sync_db)
...
async def unpublish_from_telegram(
    ...
    try:
        ...
        with get_sync_db_context() as sync_db:
            ...
            result = await portfolio_telegram_service.delete_published_item(portfolio_item, sync_db)
